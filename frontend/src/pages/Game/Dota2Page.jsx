...
const Dota2Page = () => {
    ...
    const data = gamesDetailedData?.dota2;

    if (!data) return <div style={{color: 'white', padding: '100px'}}>Cargando datos de Dota 2...</div>;

    return (
        <div className="game-page-premium theme-dota2">
            <div className="background-hero" style={{ backgroundImage: `url(${bannerDota})` }}>
            ...
            <main className="content-overlay">
                <header className="game-header">
                    ...
                    <div className="passion-tags">
                        {data.tags?.map((t, index) => <span key={index}>{t}</span>)}
                    </div>
                </header>
